  ...
  return (
    <div className="Project">
      ...
      {projectNumber && projectIndex !== null && <>
        ...
        <ProjectHeader project={projectState.projects[projectIndex]} title={projectState.projects[projectIndex]?.title} />
        <DragDropContext onDragEnd={onDragEnd}>
          {/* Columns  Droppable*/}
          <Droppable droppableId={JSON.stringify({ listName: "droppable-lists", taskNumber: undefined })} type="LIST" direction="horizontal">
            {/* Columns Draggable */}
            {(provided) => (
              <div className="Project__Columns" ref={provided.innerRef} {...provided.droppableProps}>
                {
                  Object.entries(projectState.projects[projectIndex].columns).map(([listName, column], index) =>
                    <Draggable key={listName} draggableId={JSON.stringify({ listName: listName, taskNumber: undefined })} index={index} >
                      {(provided, snapshot) => {
                        return (
                          <ProjectColumn snapshot={snapshot} provided={provided} listName={listName} projectNumber={projectNumber} changeIsAdd={changeIsAdd} column={column} />
                        );
                      }}
